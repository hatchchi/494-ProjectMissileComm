...
public class GameManager : MonoBehaviour
{
    ...
    public void TileClicked(GameObject tile)
    {
        if (setupComplete && playerTurn)
        {
            Vector3 tilePos = tile.transform.position;
            tilePos.y += 15;
            Debug.Log("tilePos is _______________ for missile_______" + tilePos);
            playerTurn = false;
            Instantiate(missilePrefab, tilePos, missilePrefab.transform.rotation);
        }
        else if (!setupComplete)
        {
            ...
            Debug.Log("tile is _______________" + tile);
        }
    }
    ...
    private void PlaceShip(GameObject tile)
    {
        ...
        Debug.Log ("__________________shipScript.GetOffsetVec(tile.transform.position)" + shipScript.GetOffsetVec(tile.transform.position));
